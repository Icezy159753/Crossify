...
// แทนที่ฟังก์ชันเก่า (รองรับทั้ง V1 และ V2 ที่อาจจะเพี้ยน)
const funcRegex = /function enableNoTopRun\(\) \{[\s\S]+?\}\n      \}/;
if (funcRegex.test(content)) {
    content = content.replace(funcRegex, newFunc);
    fs.writeFileSync(filePath, content, 'utf8');
    fs.writeFileSync('index.recovered.html', content, 'utf8');
    console.log('Successfully patched enableNoTopRun to V3 (Robust CSS + DOM)');
} else {
